<?php
	 ...
	 * listing a collection of Tecido objects.  It includes
	 ...
	 * 
	 */
	class TecidoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Tecido datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Tecido object, or it can be any QQNode extending from QQN::Tecido().
		 * 
		 ...
		 * sorting using two fields from the Tecido object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Tecido
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Tecido::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Tecido, given the clauses above
			$this->DataSource = Tecido::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Tecido-based QQNode.
		 * It will also verify that it is a proper Tecido-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tecido') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tecido".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Tecido()->Id;
