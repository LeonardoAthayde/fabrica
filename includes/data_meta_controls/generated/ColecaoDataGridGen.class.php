<?php
	 ...
	 * listing a collection of Colecao objects.  It includes
	 ...
	 * 
	 */
	class ColecaoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Colecao datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Colecao object, or it can be any QQNode extending from QQN::Colecao().
		 * 
		 ...
		 * sorting using two fields from the Colecao object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Colecao
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Colecao::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Colecao, given the clauses above
			$this->DataSource = Colecao::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Colecao-based QQNode.
		 * It will also verify that it is a proper Colecao-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'colecao') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "colecao".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Colecao()->Id;
				case 'Nome': return QQN::Colecao()->Nome;
				default: throw new QCallerException('Simple Property not found in ColecaoDataGrid content: ' . $mixContent);
