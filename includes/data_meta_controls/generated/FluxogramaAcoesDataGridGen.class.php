<?php
	 ...
	 * listing a collection of FluxogramaAcoes objects.  It includes
	 ...
	 * 
	 */
	class FluxogramaAcoesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FluxogramaAcoes datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FluxogramaAcoes object, or it can be any QQNode extending from QQN::FluxogramaAcoes().
		 * 
		 ...
		 * sorting using two fields from the FluxogramaAcoes object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FluxogramaAcoes
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = FluxogramaAcoes::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from FluxogramaAcoes, given the clauses above
			$this->DataSource = FluxogramaAcoes::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FluxogramaAcoes-based QQNode.
		 * It will also verify that it is a proper FluxogramaAcoes-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fluxograma_acoes') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fluxograma_acoes".');
