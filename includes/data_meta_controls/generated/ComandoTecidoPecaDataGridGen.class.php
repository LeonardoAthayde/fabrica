<?php
	 ...
	 * listing a collection of ComandoTecidoPeca objects.  It includes
	 ...
	 * 
	 */
	class ComandoTecidoPecaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ComandoTecidoPeca datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ComandoTecidoPeca object, or it can be any QQNode extending from QQN::ComandoTecidoPeca().
		 * 
		 ...
		 * sorting using two fields from the ComandoTecidoPeca object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ComandoTecidoPeca
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ComandoTecidoPeca::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ComandoTecidoPeca, given the clauses above
			$this->DataSource = ComandoTecidoPeca::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ComandoTecidoPeca-based QQNode.
		 * It will also verify that it is a proper ComandoTecidoPeca-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comando_tecido_peca') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comando_tecido_peca".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ComandoTecidoPeca()->Id;
				case 'Peso': return QQN::ComandoTecidoPeca()->Peso;
				case 'ComandoId': return QQN::ComandoTecidoPeca()->ComandoId;
				case 'Comando': return QQN::ComandoTecidoPeca()->Comando;
				case 'TecidoId': return QQN::ComandoTecidoPeca()->TecidoId;
				case 'Tecido': return QQN::ComandoTecidoPeca()->Tecido;
				case 'CorId': return QQN::ComandoTecidoPeca()->CorId;
				case 'Cor': return QQN::ComandoTecidoPeca()->Cor;
				default: throw new QCallerException('Simple Property not found in ComandoTecidoPecaDataGrid content: ' . $mixContent);
