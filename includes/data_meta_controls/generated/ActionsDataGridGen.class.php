<?php
	 ...
	 * listing a collection of Actions objects.  It includes
	 ...
	 * 
	 */
	class ActionsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Actions datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Actions object, or it can be any QQNode extending from QQN::Actions().
		 * 
		 ...
		 * sorting using two fields from the Actions object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Actions
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Actions::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Actions, given the clauses above
			$this->DataSource = Actions::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Actions-based QQNode.
		 * It will also verify that it is a proper Actions-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'actions') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "actions".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Actions()->Id;
				case 'Name': return QQN::Actions()->Name;
				default: throw new QCallerException('Simple Property not found in ActionsDataGrid content: ' . $mixContent);
