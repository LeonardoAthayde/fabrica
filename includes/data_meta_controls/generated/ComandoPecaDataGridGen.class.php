<?php
	 ...
	 * listing a collection of ComandoPeca objects.  It includes
	 ...
	 * 
	 */
	class ComandoPecaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ComandoPeca datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ComandoPeca object, or it can be any QQNode extending from QQN::ComandoPeca().
		 * 
		 ...
		 * sorting using two fields from the ComandoPeca object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ComandoPeca
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ComandoPeca::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ComandoPeca, given the clauses above
			$this->DataSource = ComandoPeca::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ComandoPeca-based QQNode.
		 * It will also verify that it is a proper ComandoPeca-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comando_peca') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comando_peca".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ComandoPeca()->Id;
				case 'Peso': return QQN::ComandoPeca()->Peso;
				case 'QuantidadePanos': return QQN::ComandoPeca()->QuantidadePanos;
				case 'ComandoId': return QQN::ComandoPeca()->ComandoId;
				case 'Comando': return QQN::ComandoPeca()->Comando;
				case 'TecidoId': return QQN::ComandoPeca()->TecidoId;
				case 'Tecido': return QQN::ComandoPeca()->Tecido;
				case 'CorId': return QQN::ComandoPeca()->CorId;
				case 'Cor': return QQN::ComandoPeca()->Cor;
				default: throw new QCallerException('Simple Property not found in ComandoPecaDataGrid content: ' . $mixContent);
