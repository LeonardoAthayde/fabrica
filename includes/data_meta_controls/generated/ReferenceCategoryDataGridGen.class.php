<?php
	 ...
	 * listing a collection of ReferenceCategory objects.  It includes
	 ...
	 * 
	 */
	class ReferenceCategoryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ReferenceCategory datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ReferenceCategory object, or it can be any QQNode extending from QQN::ReferenceCategory().
		 * 
		 ...
		 * sorting using two fields from the ReferenceCategory object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ReferenceCategory
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ReferenceCategory::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ReferenceCategory, given the clauses above
			$this->DataSource = ReferenceCategory::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ReferenceCategory-based QQNode.
		 * It will also verify that it is a proper ReferenceCategory-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'reference_category') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "reference_category".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ReferenceCategory()->Id;
				case 'Name': return QQN::ReferenceCategory()->Name;
				default: throw new QCallerException('Simple Property not found in ReferenceCategoryDataGrid content: ' . $mixContent);
