<?php
	 ...
	 * listing a collection of ComandoRiscoPeca objects.  It includes
	 ...
	 * 
	 */
	class ComandoRiscoPecaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ComandoRiscoPeca datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ComandoRiscoPeca object, or it can be any QQNode extending from QQN::ComandoRiscoPeca().
		 * 
		 ...
		 * sorting using two fields from the ComandoRiscoPeca object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ComandoRiscoPeca
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ComandoRiscoPeca::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ComandoRiscoPeca, given the clauses above
			$this->DataSource = ComandoRiscoPeca::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ComandoRiscoPeca-based QQNode.
		 * It will also verify that it is a proper ComandoRiscoPeca-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comando_risco_peca') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comando_risco_peca".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ComandoRiscoPeca()->Id;
				case 'QuantidadeReal': return QQN::ComandoRiscoPeca()->QuantidadeReal;
				case 'Peso': return QQN::ComandoRiscoPeca()->Peso;
				case 'ComandoRiscoId': return QQN::ComandoRiscoPeca()->ComandoRiscoId;
				case 'ComandoRisco': return QQN::ComandoRiscoPeca()->ComandoRisco;
				case 'ComandoPecaId': return QQN::ComandoRiscoPeca()->ComandoPecaId;
				case 'ComandoPeca': return QQN::ComandoRiscoPeca()->ComandoPeca;
				default: throw new QCallerException('Simple Property not found in ComandoRiscoPecaDataGrid content: ' . $mixContent);
