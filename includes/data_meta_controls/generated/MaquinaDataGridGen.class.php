<?php
	 ...
	 * listing a collection of Maquina objects.  It includes
	 ...
	 * 
	 */
	class MaquinaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Maquina datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Maquina object, or it can be any QQNode extending from QQN::Maquina().
		 * 
		 ...
		 * sorting using two fields from the Maquina object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Maquina
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Maquina::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Maquina, given the clauses above
			$this->DataSource = Maquina::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Maquina-based QQNode.
		 * It will also verify that it is a proper Maquina-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'maquina') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "maquina".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Maquina()->Id;
				case 'Nome': return QQN::Maquina()->Nome;
				default: throw new QCallerException('Simple Property not found in MaquinaDataGrid content: ' . $mixContent);
