<?php
	 ...
	 * listing a collection of Cor objects.  It includes
	 ...
	 * 
	 */
	class CorDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Cor datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Cor object, or it can be any QQNode extending from QQN::Cor().
		 * 
		 ...
		 * sorting using two fields from the Cor object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Cor
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Cor::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Cor, given the clauses above
			$this->DataSource = Cor::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Cor-based QQNode.
		 * It will also verify that it is a proper Cor-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'cor') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "cor".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Cor()->Id;
				case 'Nome': return QQN::Cor()->Nome;
				default: throw new QCallerException('Simple Property not found in CorDataGrid content: ' . $mixContent);
