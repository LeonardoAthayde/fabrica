<?php
	 ...
	 * listing a collection of Machine objects.  It includes
	 ...
	 * 
	 */
	class MachineDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Machine datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Machine object, or it can be any QQNode extending from QQN::Machine().
		 * 
		 ...
		 * sorting using two fields from the Machine object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Machine
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Machine::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Machine, given the clauses above
			$this->DataSource = Machine::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Machine-based QQNode.
		 * It will also verify that it is a proper Machine-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'machine') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "machine".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Machine()->Id;
				case 'Name': return QQN::Machine()->Name;
				case 'MachineKindId': return QQN::Machine()->MachineKindId;
				case 'MachineKind': return QQN::Machine()->MachineKind;
				default: throw new QCallerException('Simple Property not found in MachineDataGrid content: ' . $mixContent);
