<?php
	 ...
	 * listing a collection of Producao objects.  It includes
	 ...
	 * 
	 */
	class ProducaoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Producao datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Producao object, or it can be any QQNode extending from QQN::Producao().
		 * 
		 ...
		 * sorting using two fields from the Producao object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Producao
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Producao::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Producao, given the clauses above
			$this->DataSource = Producao::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Producao-based QQNode.
		 * It will also verify that it is a proper Producao-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'producao') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "producao".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Producao()->Id;
				case 'CostureiraId': return QQN::Producao()->CostureiraId;
				case 'Costureira': return QQN::Producao()->Costureira;
				case 'OrdemProducaoGradeId': return QQN::Producao()->OrdemProducaoGradeId;
				case 'OrdemProducaoGrade': return QQN::Producao()->OrdemProducaoGrade;
				case 'Quantidade': return QQN::Producao()->Quantidade;
				case 'TempoPrevisto': return QQN::Producao()->TempoPrevisto;
				case 'TempoExecutado': return QQN::Producao()->TempoExecutado;
				default: throw new QCallerException('Simple Property not found in ProducaoDataGrid content: ' . $mixContent);
