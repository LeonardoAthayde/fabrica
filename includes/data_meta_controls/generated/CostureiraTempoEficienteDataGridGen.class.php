<?php
	 ...
	 * listing a collection of CostureiraTempoEficiente objects.  It includes
	 ...
	 * 
	 */
	class CostureiraTempoEficienteDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CostureiraTempoEficiente datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CostureiraTempoEficiente object, or it can be any QQNode extending from QQN::CostureiraTempoEficiente().
		 * 
		 ...
		 * sorting using two fields from the CostureiraTempoEficiente object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CostureiraTempoEficiente
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CostureiraTempoEficiente::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CostureiraTempoEficiente, given the clauses above
			$this->DataSource = CostureiraTempoEficiente::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CostureiraTempoEficiente-based QQNode.
		 * It will also verify that it is a proper CostureiraTempoEficiente-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'costureira_tempo_eficiente') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "costureira_tempo_eficiente".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CostureiraTempoEficiente()->Id;
				case 'CostureiraProducaoId': return QQN::CostureiraTempoEficiente()->CostureiraProducaoId;
				case 'CostureiraProducao': return QQN::CostureiraTempoEficiente()->CostureiraProducao;
				case 'Date': return QQN::CostureiraTempoEficiente()->Date;
				case 'Tempo': return QQN::CostureiraTempoEficiente()->Tempo;
				default: throw new QCallerException('Simple Property not found in CostureiraTempoEficienteDataGrid content: ' . $mixContent);
