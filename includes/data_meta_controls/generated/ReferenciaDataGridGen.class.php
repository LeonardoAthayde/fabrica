<?php
	 ...
	 * listing a collection of Referencia objects.  It includes
	 ...
	 * 
	 */
	class ReferenciaDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Referencia datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Referencia object, or it can be any QQNode extending from QQN::Referencia().
		 * 
		 ...
		 * sorting using two fields from the Referencia object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Referencia
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Referencia::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Referencia, given the clauses above
			$this->DataSource = Referencia::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Referencia-based QQNode.
		 * It will also verify that it is a proper Referencia-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'referencia') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "referencia".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Referencia()->Id;
				case 'Nome': return QQN::Referencia()->Nome;
				case 'ReferenciaCategoriaId': return QQN::Referencia()->ReferenciaCategoriaId;
				case 'ReferenciaCategoria': return QQN::Referencia()->ReferenciaCategoria;
				case 'Modelo': return QQN::Referencia()->Modelo;
				case 'TecidoId': return QQN::Referencia()->TecidoId;
				case 'Tecido': return QQN::Referencia()->Tecido;
				default: throw new QCallerException('Simple Property not found in ReferenciaDataGrid content: ' . $mixContent);
