<?php
	 ...
	 * listing a collection of BalancoPecas objects.  It includes
	 ...
	 * 
	 */
	class BalancoPecasDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this BalancoPecas datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the BalancoPecas object, or it can be any QQNode extending from QQN::BalancoPecas().
		 * 
		 ...
		 * sorting using two fields from the BalancoPecas object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from BalancoPecas
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = BalancoPecas::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from BalancoPecas, given the clauses above
			$this->DataSource = BalancoPecas::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a BalancoPecas-based QQNode.
		 * It will also verify that it is a proper BalancoPecas-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'balanco_pecas') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "balanco_pecas".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::BalancoPecas()->Id;
				case 'OrdemProducaoGradeId': return QQN::BalancoPecas()->OrdemProducaoGradeId;
				case 'OrdemProducaoGrade': return QQN::BalancoPecas()->OrdemProducaoGrade;
				case 'Balanco': return QQN::BalancoPecas()->Balanco;
				case 'QuantidadeProduzida': return QQN::BalancoPecas()->QuantidadeProduzida;
				case 'Concluido': return QQN::BalancoPecas()->Concluido;
				default: throw new QCallerException('Simple Property not found in BalancoPecasDataGrid content: ' . $mixContent);
