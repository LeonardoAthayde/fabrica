<?php
	 ...
	 * listing a collection of Tissue objects.  It includes
	 ...
	 * 
	 */
	class TissueDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Tissue datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Tissue object, or it can be any QQNode extending from QQN::Tissue().
		 * 
		 ...
		 * sorting using two fields from the Tissue object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Tissue
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Tissue::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Tissue, given the clauses above
			$this->DataSource = Tissue::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Tissue-based QQNode.
		 * It will also verify that it is a proper Tissue-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tissue') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tissue".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Tissue()->Id;
				case 'Name': return QQN::Tissue()->Name;
				case 'Reference': return QQN::Tissue()->Reference;
				default: throw new QCallerException('Simple Property not found in TissueDataGrid content: ' . $mixContent);
