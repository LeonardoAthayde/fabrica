<?php
	 ...
	 * listing a collection of Molde objects.  It includes
	 ...
	 * 
	 */
	class MoldeDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Molde datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Molde object, or it can be any QQNode extending from QQN::Molde().
		 * 
		 ...
		 * sorting using two fields from the Molde object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Molde
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Molde::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Molde, given the clauses above
			$this->DataSource = Molde::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Molde-based QQNode.
		 * It will also verify that it is a proper Molde-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'molde') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "molde".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Molde()->Id;
				case 'Nome': return QQN::Molde()->Nome;
				default: throw new QCallerException('Simple Property not found in MoldeDataGrid content: ' . $mixContent);
