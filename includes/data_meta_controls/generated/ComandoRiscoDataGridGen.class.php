<?php
	 ...
	 * listing a collection of ComandoRisco objects.  It includes
	 ...
	 * 
	 */
	class ComandoRiscoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ComandoRisco datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ComandoRisco object, or it can be any QQNode extending from QQN::ComandoRisco().
		 * 
		 ...
		 * sorting using two fields from the ComandoRisco object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ComandoRisco
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = ComandoRisco::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from ComandoRisco, given the clauses above
			$this->DataSource = ComandoRisco::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ComandoRisco-based QQNode.
		 * It will also verify that it is a proper ComandoRisco-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comando_risco') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comando_risco".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ComandoRisco()->Id;
