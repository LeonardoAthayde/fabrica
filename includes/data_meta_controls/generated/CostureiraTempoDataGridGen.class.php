<?php
	 ...
	 * listing a collection of CostureiraTempo objects.  It includes
	 ...
	 * 
	 */
	class CostureiraTempoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CostureiraTempo datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CostureiraTempo object, or it can be any QQNode extending from QQN::CostureiraTempo().
		 * 
		 ...
		 * sorting using two fields from the CostureiraTempo object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CostureiraTempo
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CostureiraTempo::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CostureiraTempo, given the clauses above
			$this->DataSource = CostureiraTempo::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CostureiraTempo-based QQNode.
		 * It will also verify that it is a proper CostureiraTempo-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'costureira_tempo') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "costureira_tempo".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CostureiraTempo()->Id;
				case 'CostureiraProducaoId': return QQN::CostureiraTempo()->CostureiraProducaoId;
				case 'CostureiraProducao': return QQN::CostureiraTempo()->CostureiraProducao;
				case 'Inicio': return QQN::CostureiraTempo()->Inicio;
				case 'Fim': return QQN::CostureiraTempo()->Fim;
				case 'Tempo': return QQN::CostureiraTempo()->Tempo;
				default: throw new QCallerException('Simple Property not found in CostureiraTempoDataGrid content: ' . $mixContent);
