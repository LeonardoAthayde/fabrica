<?php
	 ...
	 * listing a collection of MachineKind objects.  It includes
	 ...
	 * 
	 */
	class MachineKindDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this MachineKind datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the MachineKind object, or it can be any QQNode extending from QQN::MachineKind().
		 * 
		 ...
		 * sorting using two fields from the MachineKind object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MachineKind
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = MachineKind::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from MachineKind, given the clauses above
			$this->DataSource = MachineKind::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MachineKind-based QQNode.
		 * It will also verify that it is a proper MachineKind-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'machine_kind') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "machine_kind".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::MachineKind()->Id;
				case 'Name': return QQN::MachineKind()->Name;
				default: throw new QCallerException('Simple Property not found in MachineKindDataGrid content: ' . $mixContent);
