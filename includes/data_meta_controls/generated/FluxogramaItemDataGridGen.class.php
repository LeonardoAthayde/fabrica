<?php
	 ...
	 * listing a collection of FluxogramaItem objects.  It includes
	 ...
	 * 
	 */
	class FluxogramaItemDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this FluxogramaItem datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FluxogramaItem object, or it can be any QQNode extending from QQN::FluxogramaItem().
		 * 
		 ...
		 * sorting using two fields from the FluxogramaItem object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FluxogramaItem
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = FluxogramaItem::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from FluxogramaItem, given the clauses above
			$this->DataSource = FluxogramaItem::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FluxogramaItem-based QQNode.
		 * It will also verify that it is a proper FluxogramaItem-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'fluxograma_item') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "fluxograma_item".');
				...
				case 'Ordenacao': return QQN::FluxogramaItem()->Ordenacao;
				...
				case 'Ativo': return QQN::FluxogramaItem()->Ativo;
