<?php
	 ...
	 * listing a collection of Comando objects.  It includes
	 ...
	 * 
	 */
	class ComandoDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Comando datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Comando object, or it can be any QQNode extending from QQN::Comando().
		 * 
		 ...
		 * sorting using two fields from the Comando object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Comando
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Comando::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Comando, given the clauses above
			$this->DataSource = Comando::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Comando-based QQNode.
		 * It will also verify that it is a proper Comando-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'comando') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "comando".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Comando()->Id;
				case 'Numero': return QQN::Comando()->Numero;
				case 'Date': return QQN::Comando()->Date;
				default: throw new QCallerException('Simple Property not found in ComandoDataGrid content: ' . $mixContent);
