<?php
	 ...
	 * listing a collection of CostureiraTempoReal objects.  It includes
	 ...
	 * 
	 */
	class CostureiraTempoRealDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CostureiraTempoReal datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CostureiraTempoReal object, or it can be any QQNode extending from QQN::CostureiraTempoReal().
		 * 
		 ...
		 * sorting using two fields from the CostureiraTempoReal object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CostureiraTempoReal
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CostureiraTempoReal::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CostureiraTempoReal, given the clauses above
			$this->DataSource = CostureiraTempoReal::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CostureiraTempoReal-based QQNode.
		 * It will also verify that it is a proper CostureiraTempoReal-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'costureira_tempo_real') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "costureira_tempo_real".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CostureiraTempoReal()->Id;
				case 'Quantidade': return QQN::CostureiraTempoReal()->Quantidade;
				case 'OrdemProducaoGradeId': return QQN::CostureiraTempoReal()->OrdemProducaoGradeId;
				case 'OrdemProducaoGrade': return QQN::CostureiraTempoReal()->OrdemProducaoGrade;
				case 'CostureiraId': return QQN::CostureiraTempoReal()->CostureiraId;
				case 'Costureira': return QQN::CostureiraTempoReal()->Costureira;
				default: throw new QCallerException('Simple Property not found in CostureiraTempoRealDataGrid content: ' . $mixContent);
