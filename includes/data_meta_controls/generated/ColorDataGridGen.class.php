<?php
	 ...
	 * listing a collection of Color objects.  It includes
	 ...
	 * 
	 */
	class ColorDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Color datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Color object, or it can be any QQNode extending from QQN::Color().
		 * 
		 ...
		 * sorting using two fields from the Color object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Color
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Color::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Color, given the clauses above
			$this->DataSource = Color::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Color-based QQNode.
		 * It will also verify that it is a proper Color-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'color') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "color".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Color()->Id;
				case 'Name': return QQN::Color()->Name;
				default: throw new QCallerException('Simple Property not found in ColorDataGrid content: ' . $mixContent);
