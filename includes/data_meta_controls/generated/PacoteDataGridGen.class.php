<?php
	 ...
	 * listing a collection of Pacote objects.  It includes
	 ...
	 * 
	 */
	class PacoteDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Pacote datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Pacote object, or it can be any QQNode extending from QQN::Pacote().
		 * 
		 ...
		 * sorting using two fields from the Pacote object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Pacote
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Pacote::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Pacote, given the clauses above
			$this->DataSource = Pacote::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Pacote-based QQNode.
		 * It will also verify that it is a proper Pacote-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'pacote') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "pacote".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Pacote()->Id;
				case 'Nome': return QQN::Pacote()->Nome;
				case 'Status': return QQN::Pacote()->Status;
				default: throw new QCallerException('Simple Property not found in PacoteDataGrid content: ' . $mixContent);
