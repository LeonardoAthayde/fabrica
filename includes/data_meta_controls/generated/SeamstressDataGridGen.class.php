<?php
	 ...
	 * listing a collection of Seamstress objects.  It includes
	 ...
	 * 
	 */
	class SeamstressDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Seamstress datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Seamstress object, or it can be any QQNode extending from QQN::Seamstress().
		 * 
		 ...
		 * sorting using two fields from the Seamstress object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Seamstress
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Seamstress::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Seamstress, given the clauses above
			$this->DataSource = Seamstress::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Seamstress-based QQNode.
		 * It will also verify that it is a proper Seamstress-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'seamstress') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "seamstress".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Seamstress()->Id;
				case 'Name': return QQN::Seamstress()->Name;
				default: throw new QCallerException('Simple Property not found in SeamstressDataGrid content: ' . $mixContent);
