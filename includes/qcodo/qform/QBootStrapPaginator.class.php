<?php
	class QBootStrapPaginator extends QPaginatorBase {
		// APPEARANCE
		...
		public function GetControlHtml() {
			...
			$strToReturn = sprintf('<span id="%s" %s%s><ul>', $this->strControlId, $strStyle, $this->GetAttributes(true, false));
	
			if ($this->intPageNumber <= 1)
				$strToReturn .= sprintf('<li class="disabled"><a href="#" >%s</a></li>', $this->strLabelForPrevious);
			else {
				$this->strActionParameter = $this->intPageNumber - 1;
				$strToReturn .= sprintf('<li><a href="" %s>%s</a></li>',
			...
			if ($this->PageCount <= $this->intIndexCount) {
				...
				if ($this->intPageNumber < $intLeftBunchTrigger) {
					...
					$strStartEllipsis = sprintf('<li><a href="" %s>%s</a></li>',
						$this->GetActionAttributes(), 1);
					$strStartEllipsis .= '<li class="disabled"><a href="#" >...</a></li>';
				}
				...
				if ($this->intPageNumber > $intRightBunchTrigger) {
					...
					$strEndEllipsis = '<li class="disabled"><a href="#" >...</a></li>';

					$this->strActionParameter = $this->PageCount;
					$strEndEllipsis .= sprintf('<li><a href="" %s>%s</a></li>',
			...
			if ($this->intPageNumber >= $this->PageCount)
				$strToReturn .= sprintf('<li class="disabled"><a href="#" >%s</a></li>', $this->strLabelForNext);
			else {
				$this->strActionParameter = $this->intPageNumber + 1;
				$strToReturn .= sprintf('<li><a href="" %s>%s</a></li>',
